...
1. Search existing issues and pull requests. For substantial changes, open an issue to discuss the approach first.
2. Keep each pull request focused. Explain the user impact, tests run, and any testing you could not perform.
3. Run the checks listed by the repository. Include device or platform testing when the change affects native behavior.
...
Maintainers may request changes for correctness, privacy, accessibility, maintainability, or release safety. A pull request is merged only after the repository's review and CI requirements are met.
